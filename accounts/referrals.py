...
Flujo:
  Cuando alguien se registra con un código de invitación:
    → El nuevo usuario recibe 1.000 créditos de bienvenida
    → El referidor recibe 1.000 créditos

Límites:
  - Máximo 4 referidos recompensados por usuario
...
def process_referral_signup(new_user, promo_code: str):
    ...
    logger.info(
        'Referral: %s usó código de %s → +%d crd a cada uno.',
        new_user.username, referrer.username, REWARD_EACH,
    )
    return True, f'¡Código aplicado! +{REWARD_EACH:,} créditos de bienvenida.'
